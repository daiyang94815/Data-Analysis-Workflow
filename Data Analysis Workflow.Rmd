...
- Goal: build parametric model for conditional distribution P(Y = k | X = x)
- A range of models use this approach
  ...
  - Naive Bayes assumes independence between features for model building (particularly useful when you have a very large number of features that are binary or categorical variables. Often seen in text classification and classification of other kind of document classification. )
...
- Assume Yi=f(Xi)+ei
- EPE(lambda)=E((Y-f_lambda_hat(X))^2)
- Suppose f_lambda_hat is the estimate from the training data and look at a new data point X=x*:  
  - E((Y-f_lambda_hat(x*))^2)=Irreducible error + Bias^2 + Variance 
  - Regularized regression can trade off bias and variance.
  
#### Hard thresholding
- Model Y=f(X)+e
- Set f_lambda_hat(x)=x\`beta (Assume that it's a linear regression model)
- Constrain only the lambda of the coefficients to be non-zero.
- Selection problem is after chosing lambda, figure out which p-lambda coefficients to make nonzero (all the possible combinations of lambda coefficients that are non-zero, and then fit the best model.)
#### Regularized regression
- To control variance, we might regularize/shrink the coefficients.
- PRSS(beta) = sum(Yj-sum(beta_1i*Xij)^2) + P(lambda; beta), where PRSS is a penalized form of the sum of squares.
- Penalty reduces complexity
- Penalty reduces variance
- Penalty respects structure of the problem
##### Ridge regression
- Solve: sum(yi-beta0+sum(xij\*betaj))^2+lambda\*sum(betaj^2), equivalent to solving sum(yi-beta0+sum(xij*betaj))^2 subject to sum(betaj)^2<=s where s is inversely proportional to lambda.
- If the beta j's squares are really big then this term will get too big, so we won't get a very good fit. This whole quantity will end up being very big, so it basically requires that some of the beta j's be small.
- Inclusion of lambda makes the problem non-singular even if X^TX is not invertible. (in that model fit where we have more predictors than observations, the ridge regression model can still be fit. )
- Tuning parameter lambda
  - lambda controls the size of the coefficients
  - lambda controls the amount of regularization
  - As lambda gets close to 0, we obtain the least square solution
  - As lambda gets close to infinity, we have betahat=0
  - picking parameter can be done with cross-validation or other techniques to try to pick the optimal tuning parameter. That trades off bias for variance. 

##### Lasso
- Solve: sum(yi-beta0+sum(xij*betaj))^2 subject to sum(abs(betaj))<=s also has a lagrangian form sum(yi-beta0+sum(xij\*betaj))^2+lambda\*sum(abs(betaj))
- For orthonormal design matrices (not the norm!) this has a closed form solution
betaj_hat=sign(betaj_hat0)(abs(betaj_hat0)-gamma)+ but not in general.
- lasso shrinks all of the coefficients and set some of them to exactly 0. And by setting something exactly to 0 it performs model selection in advance.  
...
### 6.7.8 Combining predictors (ensembling)
- Ccombine classifiers by averaging/voting (these can be classifiers that are very different. For example, combining a boosting classifier with a random forest with a linear regression model.)
...
### 6.7.9 Forecasting (Time series)
- Data are dependent over time
- Specific pattern types 
  - Trends: long term increase or decrease
  - Seasonal patterns: patterns related to time of week, month, year, etc.
  - Cycles: patterns that rise and fall periodically (non-fixed)
...
- Be careful of spurious correlations. Time series and geographic analyses can often be correlate for reasons that do not make them good for predicting one from the other. 
...
```
- `Op()` just take the opening information
...
ts1Train
```
Simple moving average: the prediction will be the average of the previous time points out to a particular time.
```{r}
plot(ts1Train)
lines(ma(ts1Train,order=3),col="red")
```
Exponential smoothing: weight near-by time points more heavily than time points that are farther away.  
```{r}
...
* Rob Hyndman's [Forecasting: principles and practice](https://www.otexts.org/fpp2/) is a good place to start
* Cautions
  * Be wary of spurious correlations
  * Be careful how far you predict (extrapolation)
  * Be wary of dependencies over time
* See [quantmod](http://cran.r-project.org/web/packages/quantmod/quantmod.pdf) or [quandl](https://www.quandl.com/tools/r) packages for finance-related problems.  
  
### 6.7.10 Unsupervised Prediction
...
```
#### Notes and further reading
- The `cl_predict()` in the {clue} provides similar functionality, in general it makes a little bit more sense to sort of build your own approach if you're doing unsupervised prediction because you really need to think carefully about how you're going to define the clusters.
...
# 7. Step 7 Developing Data Products
## 7.1 {shiny}
### 7.1.1 ui.R
- `?builder` a complete list of all the various HTML tags you can use.
  - `h1()`...`h6()`
  - `em()` emphasize
  - `code()` mono space format
  
```{r}
sliderInput(inputId, label, min, max, value)
sliderInput("slider1", "Slide Me!", 0, 100, 0)
sliderInput("sliderX", "Pick Minimum and Maximum X Values",
                  -100, 100, value = c(-50, 50)) # have the slider at the two points
```
...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      submitButton("Submit") # Delayed Reactivity. receives a labeled input value "Submit" (T/F), that you can use in your server functions.  
    ),
  mainPanel(
      plotOutput("plot1"),
...
```
#### 2. Tabs
`tabsetPanel()` specifies a group of tabs, and then `tabPanel()` specifies the contents of an individual tab.
```{r}
shinyUI(fluidPage(
  titlePanel("Tabs!"),
  sidebarLayout(
...
Select multiple data points by specifying the `brush` argument in `plotOutput()` on the `ui.R`, and then using the `brushedPoints()` function on the `server.R`.
```{r}
shinyUI(fluidPage(
...
```{r}
output$plot1 <- renderPlot({ # curly braces in reactive statements
    number_of_points <- input$numeric
    minX <- input$sliderX[1] # elements of the slider come out as a list
...
shinyServer(function(input, output) {
  mtcars$mpgsp <- ifelse(mtcars$mpg - 20 > 0, mtcars$mpg - 20, 0) # a spline term, a line and then a break point at 20 and then another line and the two lines meet continuously at that break point
...
#### Reactivity
```{r}
calc_sum <- reactive({ # curly braces in reactive statements
  input$box1 + input$box2
})

calc_sum()
```
### 7.1.3 Sharing Shiny Apps
- [shinyapps.io](http://www.shinyapps.io/) 
- If you're sharing your app with an R user you can post your app to GitHub and instruct the user to use the `runGist()` or `runGitHub()` to launch your app.  
...
- `gadgetTitleBar()` provides a title bar with Done and Cancel buttons.
- `runGadget()` runs the Shiny Gadget, taking `ui` and `server` as arguments.
```{r}
library(shiny)
library(miniUI)
...
Source the preceding code and run `pickTrees()`. Click and drag a box over the graph. Once you've drawn a box that you're satisfied with click the Done button and the points that you selected will be returned to you as a data frame. This functionality is made possible by the `brushedPoints()` function, which is part of the `shiny` package. (See `?shiny::brushedPoints` for more information.)
```{r}
pickTrees <- function() {
  ui <- miniPage(
...
Embed them in R markdown based documents: Set `results="asis"` in the chunk options.
```{r}
...
### 7.3.2 Combining multiple plots together
...
### 7.4.1 Scatterplot
Specify a scatterplot by indicating `type = "scatter"`. Notice that the arguments for the `x` and `y` variables as specified as formulas, with the tilde operator (`~`) preceding the variable that you're plotting.
```{r}
library(plotly)
...
```
### 7.4.7 Choropleth Maps
Choropleth maps illustrate data across geographic areas by shading regions with different colors.
```{r}
# Create data frame
...
```
### 7.4.8 ggplot to plotly
```{r}
gg <- ggplotly(p)
```
### 7.4.9 Post to plotly website
```{r}
plotly_POST(gg)
```
### 7.4.10 Resources
...
- chunk option `comment = ""`: drop "#"  
  
## 7.6 {leaflet} for interactive maps
```{r}
library(leaflet)
my_map <- leaflet() %>% 
  addTiles() %>%

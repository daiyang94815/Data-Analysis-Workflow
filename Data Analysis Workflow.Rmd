...
gsub("_","",testName)
```
#### 2.2.1.5 Search and find values
##### {base}
###### grep() and grepl()
```{r}
...
```
#### 2.2.1.7 Extract parts of the string / subset strings
##### {base}
```{r}
...
#### 2.2.1.9 Trim whitespace
##### {stringr}
```{r}
# removes whitespace from start and end of string
str_trim("Jeff      ")
```
#### 2.2.1.10 Regular expression
##### 2.2.1.10.1 Basic matches
...
##### 2.2.1.10.3 Character Classes with [] and alternatives
- `\d`: matches any digit.
...
str_view(x, 'C[LX]+?')
```
##### 2.2.1.10.5 Grouping and backreferences
...
```
##### 2.2.1.10.6 Other uses of regular expressions
###### apropos()
...
sentences %>% 
  str_replace("([^ ]+) ([^ ]+) ([^ ]+)", "\\1 \\3 \\2")
```
#### 2.2.1.14 Other types of pattern
##### {base}
###### regex()
